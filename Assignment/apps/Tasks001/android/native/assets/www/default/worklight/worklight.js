...
window.WLJSX.Ajax.WLRequest = WLJSX.Class.create({
  ...
  onSuccessParent: function(transport, par) {
    ...
    if (!containsChallenges) {
      ...
      if (transport.responseJSON && transport.responseJSON.notificationSubscriptionState && WL.Client.Push.__isDeviceSupportPush() && WL.Client.__isPushEnabled()) {
   ...
   */
	...
	__getHeaders = function(){
      ...
      'x-wl-clientlog-model' : model
		...
		return {'x-wl-clientlog-unified' : JSON.stringify(headers)};
	},
...
/* NOTE: The API documentation is generated from com.worklight.studio.plugin/api/wlclient.js. */
...
__WLClient = function() {
	...
	this.connect = function(options) {
		...
		function onConnectSuccess(transport) {
			...
			console.log("Calling updateConfigFromServer");
	...
	this.isUserAuthenticated = function(realm) {
	  ...
	  /**
	   * Pins single or multiple host X509 certificate(s) public key to the client application.
	   ...
	   * @param certificateFilename -  the name of the certificate file or an object containing multiple certificate file names
	   * that is located under the certificate folder located in the application root
...
this.pinTrustedCertificatePublicKey = function(certificateFilename){
	...
	this.__isPushEnabled = function() {
		return 	initOptions.enablePush;	
	}
...
/**
 * ================================================================= 
 * Source file taken from :: busyindicator.android.js
 * ================================================================= 
 */
...
WL.BusyIndicator = WLJSX.Class.create({
    ...
    show : function() {
    	...
    	cordova.exec(function(callback) {
    					if (callback) that.__visible = true;
    				 }, 
    				 function(error) {
    					 WL.Logger.debug(error);
    				 }, 
    				 "NativeBusyIndicator", "show", [this.__options.text]);  
    },

    hide : function() {
    	var that = this;
    	cordova.exec(function(callback) {
    					if (callback) that.__visible = false;
    				 },
    				 function(error) {
    					 WL.Logger.debug(error);
    				 }, 
    				 "NativeBusyIndicator", "hide", []);
    },
...
/**
 * ================================================================= 
 * Source file taken from :: ExtendedGeolocation.android.js
 * ================================================================= 
 */
...
extendedGeolocation.execGetPosition = function(id,win,fail,options) {
    cordova.exec(win, fail, "WLGeolocationPlugin", "getLocation", [id,options.enableHighAccuracy,toInt(options.maximumAge),toInt(options.desiredAccuracy)]);
};


extendedGeolocation.execAddWatch = function(id,win,fail,options) {
	cordova.exec(win, fail, "WLGeolocationPlugin", "addWatch", [id, options.enableHighAccuracy, toInt(options.desiredAccuracy),toInt(options.minChangeDistance), toInt(options.minChangeTime)]);
};

extendedGeolocation.execClearWatch = function(id) {
    cordova.exec(null, null, "WLGeolocationPlugin", "clearWatch", [id]);
};

extendedGeolocation.execRemoveCallback = function(id) {
	cordova.exec(null,null,"WLGeolocationPlugin","removeCallback",[id]);
};
...
/**
 * ================================================================= 
 * Source file taken from :: bind.android.js
 * ================================================================= 
 */
...
__WLTabBar = function() {
    ...
    this.init = function() {
        ...
        var parentElement = (parentDivId != null) ? WLJSX.$(parentDivId) : WLJSX.$$$("body");
   ...
   this.setParentDivId = function(divId) {
    ...
    var adjustTabsWidth = function() {
        ...
        var availableWidth = WLJSX.width('body');
...
__WLOptionsMenu = function() {
    ...
    /**
     * Initializes the Android OptionsMenu, enabling it, and making it visible. Must
     * be called before using it.
     * 
     * Note: If you are developing your application for Android 3.0 (API level 11) and higher, WL.OptionsMenu might have no effect, depending on the device. 
     * For more information, see http://developer.android.com/guide/topics/ui/menus.html#options-menu.
     * 
     * @param id - number, currently not used.
     *            
     */
    this.init = function() {
...
__WLPush = function() {
    ...
    function updateTokenCallback(serverToken, deviceToken, serverAppUserId, serverLoginUserId, appUserId) {
        ...
        if (updateTokenFlowRequired) {
            WL.Logger.debug("Push notification device token or the appUserId has changed, updating server notification token id."); 
            var requestOptions = {
                onSuccess : function() {
                    ...
                    if(showAll == "true") {
                		cordova.exec(null, null, 'Push', 'showAllNotifications', [showAll,wlInitOptions.autoHideSplash]);
                	}
                },
                onFailure : function() {
...
/* !! NOTE: !! The API documentation is generated from com.worklight.studio.plugin/api/wltrusteer.js. */
...
WL.Trusteer = (function() {
	 ...
	 *
     * @param onSuccess - A callback function that is called when risk-assessment data is available from the Trusteer Mobile SDK.
	 * @param onFailure - A callback function that is called when risk-assessment data is not available from the Trusteer Mobile SDK
      *                   (for example, because the SDK is not installed or initialized).
	 *
	...
	/**
	 * Optional. Initializes the Trusteer Mobile SDK that is integrated with the MobileFirst project. <br /><br />
     * <b>Note:</b> This method is applicable only when automatic initialization of the Trusteer Mobile SDK
     * is disabled by setting the TRUSTEER_AUTO_INIT property to FALSE in the client properties file
     * (<b>worklight.plist</b> for iOS or <b>client.properties</b> for Android).
	 * @param onSuccess - A callback function that is called when the Trusteer Mobile SDK is installed and initialized.
	 * @param onFailure - A callback function that is called when the Trusteer Mobile SDK fails to initialize or is not installed.
	 * @methodOf WL.Trusteer# 
	 */
	function init(onSuccess, onFailure){
		if (!WLJSX.Object.isUndefined(cordova) && WLJSX.Object.isFunction(cordova.exec)){
			cordova.exec(onSuccess, onFailure, PLUGIN_NAME, ACTION_INIT, []);
		}
	}
